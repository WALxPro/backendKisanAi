...
from utils.disease_chat import start_disease_chat, start_general_chat, send_disease_chat_message
...
@router.post("/start-general-chat")
async def start_general_chat_route(farmer_id: str = Query(...)):
    try:
        return await start_general_chat(farmer_id=farmer_id, db=db)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to start general chat: {str(e)}")
    
@router.post("/send-message")
async def send_chat_message_route(request: DiseaseChatRequest):
    try:
...
async def get_chat_history(chat_id: str, farmer_id: str = Query(...)):
    try:
        ...
        return {
            ...
            "chat_mode": chat.get("chat_mode") or chat.get("chat_type") or "disease",
...
async def get_chat_status(chat_id: str, farmer_id: str = Query(...)):
    try:
        ...
        return {
            ...
            "disease_name": chat.get("disease_name"),
            "chat_count": chat.get("chat_count", 0),
            "can_continue": True,  # no limit anymore
